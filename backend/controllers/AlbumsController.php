...
class AlbumsController extends MyController
{
    public $modelName='backend\models\Albums';
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->save())
            {
                ...
                if ($tmp != null) {
                    $saveParth='albums/'.date("Y-m").'/'.$model->id;
                    ...
                    if($saved)
                    {
                        $imageUpload->maxSize($saveParth.'/'.$imageName,Albums::MAX_IMAGE_WIDTH,Albums::MAX_IMAGE_HEIGHT);
                        $imageUpload->thumbExport($saveParth.'/'.$imageName,$saveParth.'/thumb/',$thumbName,Albums::THUMB_IMAGE_WIDTH,Albums::THUMB_IMAGE_HEIGHT);
    ...
    public function actionDelete($id)
    {
        ...
        if($info->save_mode==0)
        {
            $inner=AlbumsInner::find()->where(['parent_id'=>$id])->asArray()->all();
            $imageUpload=new ImageUpload();
            foreach($inner as $row)
            {
                $imageUpload->deleteOldImages([$row["image"],$row["thumb"]]);
                AlbumsInner::findOne($row["id"])->delete();
            }

            $imageUpload->deleteOldImages([$info->image,$info->thumb]);

            if(is_dir(MyFunctions::getImagePath().'/albums/'.date("Y-m",$info->create_time).'/'.$id.'/thumb')) rmdir(MyFunctions::getImagePath().'/albums/'.date("Y-m",$info->create_time).'/'.$id.'/thumb');
            if(is_dir(MyFunctions::getImagePath().'/albums/'.date("Y-m",$info->create_time).'/'.$id)) rmdir(MyFunctions::getImagePath().'/albums/'.date("Y-m",$info->create_time).'/'.$id);
            $info->delete();
