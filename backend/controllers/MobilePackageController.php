...
class MobilePackageController extends MyController
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())){
            ...
            if($model->save(false))
            {
				...
				$saveArchive->time_count=$model->balance; $saveArchive->operation='admin_add_package_mobile'; $saveArchive->create_time=date("Y-m-d H:i:s");
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->save(false))
            {
				...
				$saveArchive=new ArchiveDb(); $saveArchive->from_='Admin:'.$this->adminLoggedInfo[0]["id"]; $saveArchive->to_='Mobile:'.$model->mobile;
				$saveArchive->time_count=$model->balance; $saveArchive->operation='admin_change_package_mobile'; $saveArchive->create_time=date("Y-m-d H:i:s");
    ...
    public function actionDelete($id)
    {
        $find=$this->findModel($id);
			...
			$saveArchive->operation='admin_delete_package_mobile'; $saveArchive->create_time=date("Y-m-d H:i:s");
