...
class AnnouncesSearch extends Announces
{
    ...
    {
        return [
            [['id', 'room_count', 'rent_type', 'property_type', 'announce_type', 'country', 'city', 'region', 'settlement', 'metro', 'floor_count', 'current_floor', 'repair', 'document', 'view_count', 'announcer', 'insert_type', 'urgently', 'sort_search', 'sort_foward', 'sort_package', 'sort_premium','archive_view'], 'integer'],
    ...
    public function search($params,$status)
    {
        ...
        elseif($status==10) $query->where('archive_view>0');
        elseif($status==9) $query->where('id>0');
        ...
        $query->andFilterWhere([
            ...
            'archive_view' => $this->archive_view
        ]);
