...
if($status=='all') $title = 'Bütün elanlarda';
else if($status==0) $title = 'Yeni elanlarda';
    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [
                ...
                'format' => 'raw',
                'value' => function($data) {
                        if($data["status"]==0) return Html::a('<i class="fa fa-circle white"></i> Gözləyir',Url::toRoute(['status?id='.$data["id"].'&status=1']),['class'=>'btn '.MyFunctions::getStatusTemplate($data["status"]).' btn-sm']);
                        else if($data["status"]==1) return Html::a('<i class="fa fa-circle white"></i> Aktivdir',Url::toRoute(['status?id='.$data["id"].'&status=3']),['class'=>'btn '.MyFunctions::getStatusTemplate($data["status"]).' btn-sm']);
                        else if($data["status"]==2 && $data["announce_date"]!=$data["create_time"]) return Html::a('<i class="fa fa-circle white"></i> Bitib','javascript:void(0);',['class'=>'btn '.MyFunctions::getStatusTemplate($data["status"]).' btn-sm']);
                        else if($data["status"]==3) return Html::a('<i class="fa fa-circle white"></i> deAktiv',Url::toRoute(['status?id='.$data["id"].'&status=1']),['class'=>'btn '.MyFunctions::getStatusTemplate($data["status"]).' btn-sm']);
                        else if($data["status"]==4 or ($data["status"]==2 && $data["announce_date"]==$data["create_time"]) ) return Html::a('<i class="fa fa-circle white"></i> Silinib',Url::toRoute(['status?id='.$data["id"].'&status=1']),['class'=>'btn '.MyFunctions::getStatusTemplate($data["status"]).' btn-sm']);
            ...
            [
                ...
                'format' => 'raw',
                'value' => function($data){
                        $code=MyFunctions::codeGeneratorforAdmin($data["id"],Yii::$app->user->id);
                        $editURL='../elan-ver/index?id='.$data["id"].'&code='.$code.'&admin='.Yii::$app->user->id;
						...
						$premiumURL='announces-search/set_premium/'.$data["id"];
						...
						<a class="btn btn-primary btn-xs" href="'.Url::toRoute([$editURL]).'" title="Premium et" aria-label="Premium et" data-pjax="0"><span class="glyphicon glyphicon-star white"></span></a>
