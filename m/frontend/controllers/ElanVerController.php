...
class ElanVerController extends MyController
{
    ...
    public function actionCheck_phone(){
        ...
        foreach($mobiles as $mob){
            $elan_sayi=Announces::find()->select(['id'])->where(['like', 'mobile', $mob])->andWhere('status=1 or status=0 or status=3')->andWhere('insert_type=0')->count('id');
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            else
            {
                ...
                if($editingId>0)
                {
					...
					if(!is_dir(MyFunctionsF::getImagePath().'/'.$dest)) mkdir(MyFunctionsF::getImagePath().'/'.$dest,0755);
                }
                else { $oldInfo=''; $dest=''; }
...
if($property_type==7) {$insertAnnounce->current_floor=0; $insertAnnounce->floor_count=0; $insertAnnounce->repair=0;}
                        ...
                        if(!is_dir(MyFunctionsF::getImagePath().'/'.$folder)) mkdir(MyFunctionsF::getImagePath().'/'.$folder,0755);
                        if(!is_dir(MyFunctionsF::getImagePath().'/'.$folderM)) mkdir(MyFunctionsF::getImagePath().'/'.$folderM,0755);
                        if(!is_dir(MyFunctionsF::getImagePath().'/'.$folderA)) mkdir(MyFunctionsF::getImagePath().'/'.$folderA,0755);
