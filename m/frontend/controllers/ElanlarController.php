...
class ElanlarController extends MyController
{
    public function actionRedirect($id=''){
        ...
        if(empty($elan)){
            ...
            $title=$this->titleGenerator('az',$elan["announce_type"],$elan["property_type"],$elan["space"],$elan["room_count"],$elan["mark"],$elan["settlement"],$elan["metro"],$elan["region"],$elan["city"],$elan["address"]);
    ...
    public function actionIndex(){
        ...
        elseif(isset($q)){		// axtarisda soz yazarsa
            ...
            if(empty($check_info)){
                ...
                foreach($exp as $w){
                    $add_wh.="text like '%$w%' or ";
                }
                $add_wh=substr($add_wh,0,strlen($add_wh)-3);
                $query_text="select * from announces where status=1 and ($add_wh) ";
...
//            if($announce_type>0) { $query.=" and announce_type=".$announce_type; $link.='&announce_type='.$announce_type; }
        ...
        elseif($query_text==''){
            ...
            if(isset($rent_type_gun) && $rent_type_gun==1)
            {
