...
class SeoManualController extends MyController
{
    public function actionIndex(){
		...
		$key=0;		$announces=[];
		$select_q='id,cover,urgently,announce_type,property_type,space,room_count,mark,settlement,metro,region,city,address,sort_search,text,floor_count,current_floor,price,document,country';
		foreach(Yii::$app->params["fullAnnouncesArchives"] as $tb=>$table){
			
			if(isset($anns[$key])){
				if($anns[$key]>=$lazim){
					$ann_rows=Yii::$app->db->createCommand("select $select_q from $tb where ($where) and status in (1,2) order by $order limit $start,$gosterilecek")->queryAll();
			...
			/*
			$ann_rows=Yii::$app->db->createCommand("select $select_q from $tb where ($where) and status in (1,2) order by rand() desc limit $start,$gosterilecek")->queryAll();
			$announces=array_merge($announces,$ann_rows);
			if(count($announces)>=$limit) break;
			*/
		}
