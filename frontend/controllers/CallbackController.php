...
class CallbackController extends MyController
{
        ...
        if ($resp->status->code == 1 && $resp->checkCount == 0) {
            ...
            if($desc=='premium'){
                ...
                if($sort_premium>0 && $amount>0){
                    ...
                    if(!empty($announce) && $announce->sort_premium<$time){
                        $muddet=$sort_premium;
                        $aa=ceil($muddet/10);		$aa=$aa*10;		$muddet=$muddet+($aa-$muddet);
                        $announce->sort_premium=$muddet;
                        /*
                        // evvel bele idi.. premium uzerine premium etmek olurdu
            ...
            elseif($desc=='foward' || $desc=='fowardPackage'){
                $amount/=100; $sort_foward=$time+($this->packagePrices["announce_foward_time"]*3600); $time_count=($this->packagePrices["announce_foward_time"]*3600/86400);
                ...
                if($amount>0 && $paket>0){
                    ...
                    $this->userInfo->package_foward+=$paket;	$this->userInfo->save(false);
                    $this->getCacheUpdate();
            ...
            elseif($desc=='urgently'){
                ...
                if($amount==$this->packagePrices["announce_urgent"]){
                    ...
                    if(!empty($announce)){
                        ...
                        $announce->status=1;
                        $announce->announce_date=date("Y-m-d H:i:s");
                        $announce->save(false);
            ...
            elseif($desc=='mobile'){
                ...
                if($amount>0 && $paket>0 && $mobile!=''){
                    ...
                    $saveMobile->create_time=date("Y-m-d H:i:s");
            ...
            elseif($desc=='searchFoward' || $desc=='searchFowardPackage'){
                $amount/=100; $sort_search=$time+(10*86400); $time_count=10;
                if($amount==$this->packagePrices["announce_search10"]){
                    if($desc=='searchFowardPackage') $paket=1; else $paket=0;
                    if($paket>0){
                        $saveArchive=new ArchiveDb();
                        $saveArchive->from_='User:'.$this->userInfo->id;
                        $saveArchive->to_='User:'.$this->userInfo->id;
                        $saveArchive->operation='profil_get_searchFowardPackage';
                        $saveArchive->time_count=$paket;
                        $saveArchive->announce_id=$id;
                        $saveArchive->price=$amount;
                        $saveArchive->create_time=date("Y-m-d H:i:s");
                        ...
                        $this->userInfo->package_search+=$paket;	$this->userInfo->save(false);
                        $this->getCacheUpdate();
                        Yii::$app->session->setFlash('success',Yii::t('app','lang272'));
                        echo "<script type='text/javascript'>window.opener.refreshParent();window.close();</script>";
                    }
                        ...
                        if( !empty($announce) ){  //  && $announce->sort_search<$time		// bunu cixartdim cunki axtarisda ireli cekilmiwi bir daha etmek olsun deye
                            $muddet=$sort_search;
                            $aa=ceil($muddet/10);		$aa=$aa*10;		$muddet=$muddet+($aa-$muddet);
                            $announce->sort_search=$muddet;
                            /*
                            // evvel bele idi.. emeliyat uzerine emeliyat etmek olurdu
                        ...
                        else echo "<script type='text/javascript'>window.opener.refreshParent();window.close();</script>";
                    }
                }
                else echo "<script type='text/javascript'>window.close();</script>";
            }
            elseif($desc=='share'){
                $amount/=100;
                if($amount==$this->packagePrices["announce_fb"]){
                    foreach(Yii::$app->params["fullAnnouncesArchives"] as $tb_name=>$table){
                        $tb='backend\models\\'.$table;
                        $announce=$tb::find()->where(['id'=>$id])->one();
                        if(!empty($announce)) break;
            ...
            elseif($desc=='all' || $desc=='allreklam'){
            ...
            elseif($desc=='archiveAnn'){
                $amount/=100; $time_count=30;
                if($amount==2){
                    foreach(Yii::$app->params["fullAnnouncesArchives"] as $tb_name=>$table){
                        $tb='backend\models\\'.$table;
                        $announce=$tb::find()->where(['id'=>$id])->one();
                    ...
                    if(!empty($announce)){
                        ...
                        $saveArchive->to_='Announce:'.$id;
                        $saveArchive->operation='archive_ann_contact';
                        $saveArchive->mobiles=$announce->mobile;
                        $saveArchive->time_count=$time_count;
                        $saveArchive->announce_id=$id;
                        $saveArchive->price=$amount;
                        ...
                        $this->userInfo->archive_ann.=$id.'-';	$this->userInfo->save(false);
                        $this->getCacheUpdate();
                        Yii::$app->session->setFlash('success',"Artıq bu elanın sahibinin əlaqə vasitəsi sizin üçün görünür");
    ...
    public function actionFullbuy($id,$foward,$search,$status){
        if($this->userInfo!=false){
            if($foward>0){
                ...
                if(empty($announce)){
                    ...
                    return $this->redirect(['profil/elanlar?status='.$status]);
                }
                else{
            ...
            if($search>0){
                ...
                if(empty($announce)){
					...
					$announce->sort_search=$muddet;		// deyiwildi qaydalar. bundan sonra nece defe etsede yeniden 10 gun verecek cemi. gunler toplanmayacaq yeni
                    $announce->save(false);

                    $saveArchive=new ArchiveDb();
                    $saveArchive->from_='User:'.$this->userInfo->id;
                    $saveArchive->to_='Announce:'.$id;
                    $saveArchive->operation='user_do_searchFoward_with_Fullbuy';
                    $saveArchive->mobiles=$announce->mobile;
    ...
    public function actionFoward(){
        if(Yii::$app->request->post() && $this->userInfo!=false){
            ...
            if(empty($announce)){
                ...
                $time=time();
				if($announce->sort_foward<$time-25){	// eyni emeliyyati 25 saniyeden tez ede bilmesin
					$this->userInfo->package_foward-=1; $this->userInfo->save(false);
    ...
    public function actionSearch(){
        if(Yii::$app->request->post() && $this->userInfo!=false){
            ...
            if(empty($announce)){
				...
				$time=time();
				if($announce->sort_search<$time-25){	// eyni emeliyyati 25 saniyeden tez ede bilmesin
					$this->userInfo->package_search-=1; $this->userInfo->save(false);
