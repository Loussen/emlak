...
class ProfilController extends MyController
{
	...
	public function actionReklam($id){
		...
		return $this->renderPartial('reklam',[
			'elan'=>$announce,
			'id'=>$id,
		]);
    }
    ...
    public function actionElanlar()
    {
		...
		if(intval(Yii::$app->request->post('announce_search10'))>0 || intval(Yii::$app->request->post('announce_foward1'))>0){
			
			$current_ann=intval(Yii::$app->request->post('current_ann'));
			$foward=intval(Yii::$app->request->post('announce_foward1'));
			$search=intval(Yii::$app->request->post('announce_search10'));
			return $this->redirect(Url::toRoute(['callback/fullbuy?id='.$current_ann.'&foward='.$foward.'&search='.$search.'&status='.$status]));
		}
			...
			foreach(Yii::$app->params["fullAnnouncesArchives"] as $tb=>$table){
				...
				if(isset($this->userAnnounces2_rows[$key])){
					if($this->userAnnounces2_rows[$key]>=$lazim){
                        ...
                        if($this->userAnnounces2_rows[$key]>$minLazim)
                        {
                            $ann_rows=Yii::$app->db->createCommand("select * from $tb where email='".$this->userInfo->email."' and status='$status' order by announce_date desc limit $start,$bitmis_limit")->queryAll();
                            $ended_ann=array_merge($ended_ann,$ann_rows);
//                            $qaliq=$lazim-$this->userAnnounces2_rows[$key];  // 5 catmir tutaqki
                            $bitmis_limit-=count($ann_rows);
                            $start=0;
//                            $bitmis_limit=$qaliq;
                            $lazim=$start+$bitmis_limit;
                        }
                        else{
                            $start=$minLazim-$this->userAnnounces2_rows[$key];
                            $minLazim=$start;
                        }
            ...
            foreach(Yii::$app->params["fullAnnouncesArchives"] as $tb=>$table){
                ...
                if(isset($this->userAnnounces4_rows[$key])){
                    if($this->userAnnounces4_rows[$key]>=$lazim){
                        $ann_rows=Yii::$app->db->createCommand("select * from $tb where email='".$this->userInfo->email."' and status='$status' order by deleted_time desc limit $start,$bitmis_limit")->queryAll();
                        $ended_ann=array_merge($ended_ann,$ann_rows);
                        break;
                    }
                    else{
                        if($this->userAnnounces4_rows[$key]>$minLazim)
                        {
                            $ann_rows=Yii::$app->db->createCommand("select * from $tb where email='".$this->userInfo->email."' and status='$status' order by deleted_time desc limit $start,$bitmis_limit")->queryAll();
                            $ended_ann=array_merge($ended_ann,$ann_rows);
//                            $qaliq=$lazim-$this->userAnnounces2_rows[$key];  // 5 catmir tutaqki
...
//                            $bitmis_limit=$qaliq;
                            ...
                            $start=$minLazim-$this->userAnnounces4_rows[$key];
                            $minLazim=$start;
                        }
        ...
        if(Yii::$app->request->post('edit_submit'))
        {
            ...
            else
            {
                ...
                if($email!=$this->userInfo->email)
                {
                    ...
                    <a href="https://emlak.az/qeydiyyat/change_email?code='.$code.'&answer=1" target="_blank">'.Yii::t('app','lang112').'</a><br />
                    <a href="https://emlak.az/qeydiyyat/change_email?code='.$code.'&answer=0" target="_blank">'.Yii::t('app','lang113').'</a>';
                    ...
                    $insert->save();
                    $mesaj=Yii::t('app','lang111').'<br /> <br />
                    <a href="https://emlak.az/qeydiyyat/change_email?code='.$code2.'&answer=1" target="_blank">'.Yii::t('app','lang112').'</a><br />
                    <a href="https://emlak.az/qeydiyyat/change_email?code='.$code2.'&answer=0" target="_blank">'.Yii::t('app','lang113').'</a>';
